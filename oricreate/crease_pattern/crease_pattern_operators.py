...
def get_gamma_du(a, a_du, b, b_du):
    ...
    ab_du = np.einsum('...Ied,...d->...Ie', a_du, b) + np.einsum('...dIe,...d->...Ie', b_du, a)
    sqrt_aa_du = 1 / sqrt_aa * np.einsum('...dIe,...d->...Ie', a_du, a)
    sqrt_bb_du = 1 / sqrt_bb * np.einsum('...dIe,...d->...Ie', b_du, b)
    sqrt_aa_x_sqrt_bb_du = (np.einsum('...Ie,...->...Ie', sqrt_aa_du, sqrt_bb) +
                            np.einsum('...Ie,...->...Ie', sqrt_bb_du, sqrt_aa))
...
def get_theta_du(a, a_du, b, b_du):
    ...
    sqrt_aa = np.sqrt(np.einsum('...d,...d->...', a, a))
    sqrt_bb = np.sqrt(np.einsum('...d,...d->...', b, b))
    aa_bb = (sqrt_aa * sqrt_bb)
    gamma = ab / (aa_bb)
    d_atb = (np.einsum('...i,...de,...d->...ie', a_du, DELTA, b) +
             np.einsum('...i,...de,...d->...ie', b_du, DELTA, a))
    gamma_bb__aa = gamma * sqrt_bb / sqrt_aa
    gamma_aa__bb = gamma * sqrt_aa / sqrt_bb
    d_gamma_aa_bb = (np.einsum('...,...i,...de,...d->...ie',
                               gamma_bb__aa, a_du, DELTA, a) +
                     np.einsum('...,...i,...de,...d->...ie',
                               gamma_aa__bb, b_du, DELTA, b))
    gamma_du = np.einsum('...,...id->...id', 1. / aa_bb, (d_atb - d_gamma_aa_bb))
    sqarg = 1 - gamma ** 2
    theta_du = np.einsum('...,...id->...id', -1. / np.sqrt(sqarg), gamma_du)
    return theta_du
...
def get_theta_du2(a, a_du, b, b_du):
    ...
    d_gamma_aa_bb = (np.einsum('...,...iKj,...i->...Kj',
                     ...
                     np.einsum('...,...iKj,...i->...Kj',
                               gamma_aa__bb, b_du, b))
    gamma_du = np.einsum('...,...Kj->...Kj', 1. / aa_bb, (ab_du - d_gamma_aa_bb))
    sqarg = 1 - gamma ** 2
