...
class BarrellVaultGravityFormingProcess(HasTraits):
    ...
    @cached_property
    def _get_ctf(self):
        return [r_, s_, -0.2 * (r_ * (1 - r_ / self.L_x))]
    ...
    def _get_load_task(self):
        ...
        F_ext_list = [(n, 2, FN(0.001)) for n in cp.N_h[6, :]]
...
if __name__ == '__main__':
    bsf_process = BarrellVaultGravityFormingProcess(
        L_x=6.0, L_y=0.8, n_x=12, n_y=4, n_steps=1, u_x=0.1)
...
#    ft.formed_object.viz3d.set(tube_radius=0.001)
